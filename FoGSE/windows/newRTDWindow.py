...
class RTDWindow(QWidget):
    ...
    def update_plot(self):
        ...
        *`process_data` to perform any last steps before updating the plot.
        ...
        print(self.reader.collection.new_data)

        if len(new_data['ti'])==0:
            return
        
        chip1_values, chip2_values = self._separate_chips(new_data)
        
        # defined how to add/append onto the new data arrays
        self.chip1.add_plot_data(chip1_values, new_data_xs=list(new_data["ti"]), replace={"this":[0], "with":[np.nan]})
        self.chip2.add_plot_data(chip2_values, new_data_xs=list(new_data["ti"]), replace={"this":[0], "with":[np.nan]})
